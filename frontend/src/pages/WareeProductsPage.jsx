...
import { wareeCategories } from "../data/wareeProduct";

const WareeProductsPage = () => {
  const { categoryId } = useParams();

  const activeCategory = categoryId
    ? wareeCategories.find((c) => c.id === categoryId)
    : wareeCategories[0];
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="text-gray-500 mt-2">
            Explore premium Waaree on-grid solar kit solutions.
          </p>
        </motion.div>

        {/* Category Tabs */}
        <div className="flex justify-center border-b border-gray-200 mb-12 relative">
          {wareeCategories.map((cat) => (
            <NavLink
              key={cat.id}
              to={`/waree-products/${cat.id}`}
              className={({ isActive }) =>
            ...
            >
              ...
              {cat.id === activeCategory.id && (
                <motion.div
                  layoutId="wareeProductTabUnderline"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeCategory.items.map((item) => (
                <Link
                  key={item.id}
                  to={`/waree-products/${activeCategory.id}/${item.id}`}
...
export default WareeProductsPage;
