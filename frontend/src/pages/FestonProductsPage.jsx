...
const FestonProductsPage = () => {
  const { categoryId } = useParams();

  const activeCategory = categoryId
    ? festonCategories.find((c) => c.id === categoryId)
    : festonCategories[0];
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="text-gray-500 mt-2">
            Explore premium Feston energy storage solutions.
          </p>
        </motion.div>

        {/* Category Tabs */}
        <div className="flex justify-center border-b border-gray-200 mb-12 relative">
          {festonCategories.map((cat) => (
            <NavLink
              key={cat.id}
              to={`/feston-products/${cat.id}`}
              className={({ isActive }) =>
            ...
            >
              ...
              {cat.id === activeCategory.id && (
                <motion.div
                  layoutId="festonProductTabUnderline"
        ...
        {/* Products Grid — same layout as Enzolt Energy */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeCategory.items.map((item) => (
                <Link
                  key={item.id}
                  to={`/feston-products/${activeCategory.id}/${item.id}`}
                ...
                >
                  ...
                  {/* Know More — same position as "View Details →" */}
...
export default FestonProductsPage;
